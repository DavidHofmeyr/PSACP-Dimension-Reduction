...
PCA_reconstruction <- function(X, ncomp, loss, dloss, V0 = NULL) {
  ...
  if (is.null(V0))
    V <- matrix(rnorm(p * ncomp), ncol = ncomp)
  else
    V <- V0
...
PCA_proj_pursuit <- function(X, ncomp, loss, dloss, V0 = NULL) {
  ...
  for (j in 1:ncomp) {
    ...
    v <-
      optim(V[, j] / sqrt(sum(V[, j] ^ 2)), obj, dobj, Xc, method = 'BFGS')$par
...
### introduce a random projection matrix of dimension pp, which we can vary
pp <- 200
R <- matrix(rnorm(600*pp), ncol = pp)/sqrt(600)

hist(dist(yale$x)/dist(yale$x%*%R)*sqrt(pp/600))
...
loss_abs <- function(x)
  abs(x)
dloss_abs <- function(x)
  sign(x)

loss_log <- function(x)
  log(1 + x ^ 2)
dloss_log <- function(x)
  2 * x / (1 + x ^ 2)
...
system.time(PC_recon_sq <-
              PCA_reconstruction(Xtr, 2, loss_sq, dloss_sq, V0))

system.time(PC_pp_sq <-
              PCA_proj_pursuit(Xtr, 2, loss_sq, dloss_sq, V0))

system.time(PC_recon_abs <-
              PCA_reconstruction(Xtr, 2, loss_abs, dloss_abs, V0))

system.time(PC_pp_abs <-
              PCA_proj_pursuit(Xtr, 2, loss_abs, dloss_abs, V0))

system.time(PC_recon_log <-
              PCA_reconstruction(Xtr, 2, loss_log, dloss_log, V0))

system.time(PC_pp_log <-
              PCA_proj_pursuit(Xtr, 2, loss_log, dloss_log, V0))

rV0 <- t(R)%*%V0

system.time(PC_recon_sq_rp <-
              PCA_reconstruction(Xtr%*%R, 2, loss_sq, dloss_sq, rV0))

system.time(PC_pp_sq_rp <-
              PCA_proj_pursuit(Xtr%*%R, 2, loss_sq, dloss_sq, rV0))

system.time(PC_recon_abs_rp <-
              PCA_reconstruction(Xtr%*%R, 2, loss_abs, dloss_abs, rV0))

system.time(PC_pp_abs_rp <-
              PCA_proj_pursuit(Xtr%*%R, 2, loss_abs, dloss_abs, rV0))

system.time(PC_recon_log_rp <-
              PCA_reconstruction(Xtr%*%R, 2, loss_log, dloss_log, rV0))

system.time(PC_pp_log_rp <-
              PCA_proj_pursuit(Xtr%*%R, 2, loss_log, dloss_log, rV0))
...
plot(Xte%*%R%*%PC_recon_sq_rp, xaxt = 'n', yaxt = 'n', main = 'Reconstruction. Squared loss')
plot(Xte%*%R%*%PC_recon_abs_rp, xaxt = 'n', yaxt = 'n', main = 'Reconstruction. Absolute loss')
plot(Xte%*%R%*%PC_recon_log_rp, xaxt = 'n', yaxt = 'n', main = 'Reconstruction. Log loss')
plot(Xte%*%R%*%PC_pp_sq_rp, xaxt = 'n', yaxt = 'n', main = 'Projection pursuit. Squared loss')
plot(Xte%*%R%*%PC_pp_abs_rp, xaxt = 'n', yaxt = 'n', main = 'Projection pursuit. Absolute loss')
plot(Xte%*%R%*%PC_pp_log_rp, xaxt = 'n', yaxt = 'n', main = 'Projection pursuit. Log loss')
...
image(matrix(Xte[100,600:1], ncol = 30, byrow = T), main = 'Original', col = rgb(1:100/100, 1:100/100, 1:100/100), xaxt = 'n', yaxt = 'n')
image(matrix((Xte[100,]%*%R%*%PC_recon_sq_rp%*%t(PC_recon_sq_rp)%*%t(R))[600:1], ncol = 30, byrow = T), main = 'Reconstruction. Squared loss', col = rgb(1:100/100, 1:100/100, 1:100/100), xaxt = 'n', yaxt = 'n')
image(matrix((Xte[100,]%*%R%*%PC_recon_abs_rp%*%t(PC_recon_abs_rp)%*%t(R))[600:1], ncol = 30, byrow = T), main = 'Reconstruction. Abs loss', col = rgb(1:100/100, 1:100/100, 1:100/100), xaxt = 'n', yaxt = 'n')
image(matrix((Xte[100,]%*%R%*%PC_recon_log_rp%*%t(PC_recon_log_rp)%*%t(R))[600:1], ncol = 30, byrow = T), main = 'Reconstruction. Log loss', col = rgb(1:100/100, 1:100/100, 1:100/100), xaxt = 'n', yaxt = 'n')
image(matrix(Xte[100,600:1], ncol = 30, byrow = T), main = 'Original', col = rgb(1:100/100, 1:100/100, 1:100/100), xaxt = 'n', yaxt = 'n')
image(matrix((Xte[100,]%*%R%*%PC_pp_sq_rp%*%t(PC_pp_sq_rp)%*%t(R))[600:1], ncol = 30, byrow = T), main = 'Projection pursuit. Squared loss', col = rgb(1:100/100, 1:100/100, 1:100/100), xaxt = 'n', yaxt = 'n')
image(matrix((Xte[100,]%*%R%*%PC_pp_abs_rp%*%t(PC_pp_abs_rp)%*%t(R))[600:1], ncol = 30, byrow = T), main = 'Projection pursuit. Abs loss', col = rgb(1:100/100, 1:100/100, 1:100/100), xaxt = 'n', yaxt = 'n')
image(matrix((Xte[100,]%*%R%*%PC_pp_log_rp%*%t(PC_pp_log_rp)%*%t(R))[600:1], ncol = 30, byrow = T), main = 'Projection pursuit. Log loss', col = rgb(1:100/100, 1:100/100, 1:100/100), xaxt = 'n', yaxt = 'n')
